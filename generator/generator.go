...
import (
	...
	"gitlab.wallstcn.com/opensource/goswgspecgen/markup"
	"gitlab.wallstcn.com/opensource/goswgspecgen/parser"
)
...
func generateSwaggerUiFiles(parser *parser.Parser, outputSpec string) error {
	...
	metaJson := parser.GetResourceListingJson()
	...
	fd.WriteString(string(metaJson))
	...
	for apiKey, apiDescription := range parser.TopLevelApis {
		...
		//for _,v :=range(apiDescription.Apis){
