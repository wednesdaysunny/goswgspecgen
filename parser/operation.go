...
func (operation *Operation) ParseComment(comment string) error {
	c := strings.TrimSpace(strings.TrimRight(strings.TrimLeft(comment, "/*"), "*/"))
	r := regexp.MustCompile("@")
	is := r.FindAllIndex([]byte(c), -1)
	cs := make([]string, 0)
	p := 0
	for _, v := range is {
		cs = append(cs, c[p:v[0]])
		p = v[0]
	}
	cs = append(cs, c[p:])
	xs := make([]string, 0)
	...
	if len(xs) > 1 {
		for _, v := range xs {
			if err := operation.ParseComment(v); err != nil {
				return err
			}
		}
		return nil
