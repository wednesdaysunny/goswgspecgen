...
func (m *Model) ParseModel(modelName string, currentPackage string, knownModelNames map[string]bool) (error, []*Model) {
	...
	//if modelName == "protoimpl.SizeCache" || modelName == "protoimpl.MessageState" || modelName == "protoimpl.UnknownFields" {
	//	return nil
	//}
...
func (m *Model) ParseModelProperty(field *ast.Field, modelPackage string) {
	...
	if strings.HasPrefix(typeAsString, "[]") {
		spect, _, specdv := specType(typeAsString[2:])
		property.Type = "array"
		property.SetItemType(spect)
		switch v := specdv.(type) {
		case string:
			property.DefaultValue = []string{v}
		default:
		}
	} else {
		spect, specf, specdv := specType(typeAsString)
		property.Type = spect
		property.Format = specf
		property.DefaultValue = specdv
	}
...
func IsSpecType(typeName string) bool {
	_, ok := specTypes[typeName]
	return ok
}
...
func specType(t string) (spect string, specf string, specdv interface{}) {
	switch {
	case t == "uint64":
		fallthrough
	case t == "int64":
